/**
 * ESP32 Bewässerungssteuerung (8 Kanäle)
 * - Manuell + Gruppen-Pläne (sequentielle Schritte), Queue, Winter-Entwässerung
 * - WLAN STA + AP-Konfig (Captive-Portal, QR), Hostname/mDNS
 * - Web-UI: Seiten Start/Pläne/Konfig/Winter, dunkles Theme, Auto-Refresh,
 *   Kanalnamen, Plan-Gruppen mit Schritten, Export/Import, API-Token-Verwaltung
 * Frameworks: Arduino-ESP32
 * Libraries:  Adafruit_GFX, Adafruit_SSD1306, QRCode (ricmoo)
 */
...
static const uint8_t  CHANNEL_PINS[8]   = { 13, 14, 25, 26, 27, 32, 33, 23 };
...
static const char* WIFI_SSID = "YourSSID";          // Fallback-Default
static const char* WIFI_PASS = "YourPassword";      // Fallback-Default
...
static const uint16_t MAX_RUNTIME_SEC     = 7200;   // 120 min Obergrenze
static const uint16_t MIN_GAP_SEC         = 5;
static const bool     SINGLE_CHANNEL_MODE = true;   // Bedingung für sequentielle Gruppen!
...
static const uint8_t  API_TOKEN_LEN  = 32;          // Hex-Zeichen
...
std::queue<WateringTask> taskQueue;
...
struct Group {
  ...
  uint8_t   hour;
  uint8_t   minute;
  uint8_t   daysMask;                 // bit0=So..bit6=Sa
  uint8_t   stepCount;
  GroupStep steps[MAX_STEPS];
  uint32_t  lastRunDay;               // RAM
  uint16_t  lastRunMin;               // RAM
};
...
void stopChannel(uint8_t ch) {
  ...
  if (channels[ch].active) {
    ...
    Serial.printf("[%s] STOP\n", channelName(ch).c_str());
  }
  channels[ch].active = false;
}
void stopAllChannels() { for (uint8_t i = 0; i < 8; i++) stopChannel(i); }
...
void stopWinterization() { if (!winterActive) return; winterActive = false; stopAllChannels(); }
...
void checkGroups() {
  ...
  for (uint8_t g = 0; g < MAX_GROUPS; g++) {
    ...
    for (uint8_t k = 0; k < G.stepCount; k++)
      enqueueTask(G.steps[k].channel, G.steps[k].durationSec);
...
// ============================================================
//
// Export enthält: SSID (nur als Referenz), Kanalnamen, Gruppen-Pläne.
// KEIN WLAN-Passwort. Beim Import wird die SSID NICHT angewendet, da ohne
// Passwort keine nutzbare Verbindung herstellbar ist.
...
int applyImport(const String& txt) {
  ...
  while (start < (int)txt.length()) {
    ...
    if (line.startsWith("WIFI|")) {
      // SSID nur informativ; ohne Passwort nicht anwendbar -> übersprungen.
      continue;
    } else if (line.startsWith("NAME|")) {
      int idx = getField(line,1).toInt(); String nm = getField(line,2);
      ...
      strncpy(G.name, nm.c_str(), GROUP_NAME_MAX); G.name[GROUP_NAME_MAX] = '\0';
      G.stepCount = 0;
      applied++;
...
void startMDNS() {
  if (MDNS.begin(DEVICE_HOSTNAME)) { MDNS.addService("http","tcp",80); Serial.printf("[mDNS] %s.local\n", DEVICE_HOSTNAME); }
...
void wifiBeginSTA() {
  ...
  if (WiFi.status() == WL_CONNECTED) { Serial.printf("\n[WiFi] OK IP=%s\n", WiFi.localIP().toString().c_str()); startMDNS(); }
  else Serial.println("\n[WiFi] FEHLGESCHLAGEN");
  currentMode = OP_STA;
}
void wifiStartAP() {
...
void maintainWifi() {
  ...
  if (WiFi.status() != WL_CONNECTED) { WiFi.disconnect(); WiFi.begin(staSsid, staPass); }
}
...
void initTime() {
  ...
  struct tm ti;
  if (getLocalTime(&ti, 8000)) { ntpSynced = true; Serial.println("[NTP] sync"); }
  else Serial.println("[NTP] noch nicht sync");
}
String nowTimeStr() {
  if (!ntpSynced) return "--";
  struct tm ti; if (!getLocalTime(&ti, 50)) return "--";
  char buf[20]; strftime(buf, sizeof(buf), "%d.%m. %H:%M:%S", &ti);
  return String(buf);
}
...
int drawWrapped(int x, int y, int wpx, const String& s) {
  ...
  int maxChars = wpx / charW; if (maxChars < 1) maxChars = 1;
  for (int i = 0; i < (int)s.length(); i += maxChars) {
    display.setCursor(x, y); display.print(s.substring(i, i+maxChars)); y += lineH;
  }
  return y;
}
void showQRCodeForAP() {
  String s = String("WIFI:T:WPA;S:") + AP_SSID + ";P:" + AP_PASS + ";;";
  QRCode qr; uint8_t buffer[qrcode_getBufferSize(3)];
  qrcode_initText(&qr, buffer, 3, ECC_LOW, s.c_str());
  display.clearDisplay();
  const int scale = 2, qrPx = qr.size*scale, offX = 0;
  int offY = (OLED_HEIGHT - qrPx)/2; if (offY < 0) offY = 0;
  for (uint8_t y = 0; y < qr.size; y++)
    for (uint8_t x = 0; x < qr.size; x++)
      if (qrcode_getModule(&qr, x, y))
        display.fillRect(offX+x*scale, offY+y*scale, scale, scale, SSD1306_WHITE);
  const int tx = offX+qrPx+4, tw = OLED_WIDTH-tx; int ty = 0;
  display.setTextSize(1); display.setTextColor(SSD1306_WHITE);
  display.setCursor(tx, ty); display.print("AP-Login"); ty += 10;
  display.setCursor(tx, ty); display.print("Netz:");    ty += 8;
  ty = drawWrapped(tx, ty, tw, String(AP_SSID));
  display.setCursor(tx, ty); display.print("Pass:");    ty += 8;
...
void updateStatusDisplay() {
  ...
  if (winterActive) {
    ...
    if (winterPhase==WP_BLOW){ display.print(channelShort(winterChannel)); display.printf(" %s\n",ph); }
  ...
  display.println(WiFi.status()==WL_CONNECTED ? WiFi.localIP().toString() : "WiFi: off");
  display.println(ntpSynced ? nowTimeStr() : "Keine Zeit");
  bool any = false;
  for (uint8_t i = 0; i < 8; i++)
    if (channels[i].active) {
      uint32_t rem = (channels[i].durationMs-(millis()-channels[i].startMs))/1000;
      display.print(channelShort(i)); display.printf(" %lus\n",(unsigned long)rem); any = true;
    }
...
pre{white-space:pre-wrap;background:var(--panel2);padding:10px;border-radius:8px;border:1px solid var(--border)}
 ...
 try{
  ...
  if(ov){
   ...
   let l=['Gerät: '+esc(d.host),'Zeit: '+esc(d.time),
     'WLAN: '+esc(d.wifi)+(d.ip?(' ('+esc(d.ip)+')'):''),
     'Aktiv: '+(act.length?act.join(', '):'keine'),'Queue: '+d.queue];
...
String pageHead(const String& title, const String& active) {
  ...
  h += themeRoot(); h += FPSTR(STYLE_BASE); h += "</style></head><body><div class='wrap'>";
...
String pageFoot(bool withScript) {
  String h; if (withScript) h += FPSTR(STATUS_SCRIPT);
  h += "</div></body></html>"; return h;
}
String durationSelect(const String& field, const String& formAttr = "") {
  ...
  for (uint8_t i = 0; i < RUNTIME_OPTIONS_CNT; i++) {
    uint16_t m = RUNTIME_OPTIONS_MIN[i]; uint32_t sec = (uint32_t)m*60UL;
...
String intSelect(const String& field, const String& id, int lo, int hi, int sel, bool pad) {
  ...
  for (int v = lo; v <= hi; v++) {
    char b[4]; if (pad) snprintf(b,4,"%02d",v); else snprintf(b,4,"%d",v);
...
String groupSlotSelect() {
  ...
  for (int i = 0; i < MAX_GROUPS; i++) {
    ...
    if (groups[i].name[0] || groups[i].stepCount) {
      char t[8]; snprintf(t,8,"%02u:%02u", groups[i].hour, groups[i].minute);
...
String buildHome() {
  ...
  h += "<div>WLAN: " + String(WiFi.status()==WL_CONNECTED?"STA":(currentMode==OP_AP?"AP":"off"));
  if (WiFi.status()==WL_CONNECTED) h += " (" + WiFi.localIP().toString() + ")";
  h += "</div></div></div>";
...
String buildPlans(const String& msg = "") {
  ...
  h += "<div class='card'><h2>Gruppe anlegen / bearbeiten</h2><form action='/setgroup' method='POST'>";
  ...
  for (int g = 0; g < MAX_GROUPS; g++) {
    ...
    if (!G.enabled && G.stepCount == 0 && !G.name[0]) continue;
    any = true;
    char t[8]; snprintf(t,8,"%02u:%02u", G.hour, G.minute);
  ...
  for (int g = 0; g < MAX_GROUPS; g++) {
    Group &G = groups[g];
    if (g) data += ",";
    String nm = String(G.name); nm.replace("\\","\\\\"); nm.replace("'","\\'");
...
String buildConfig(const String& msg = "") {
  ...
  h += "<div class='muted'>WLAN: " + String(WiFi.status()==WL_CONNECTED?"STA":(currentMode==OP_AP?"AP":"off"));
  if (WiFi.status()==WL_CONNECTED) h += " · IP " + WiFi.localIP().toString();
  h += "</div><div class='muted mt'>WLAN-Zugangsdaten nur im AP-Konfig-Modus änderbar (BOOT-Taste 3 s).</div></div>";
  ...
  h += "<p class='muted'>Tokens werden maskiert angezeigt; der Klartext erscheint nur einmal direkt nach dem Erzeugen. "
  ...
  h += "<p class='muted'>Export/Import umfasst SSID (nur als Referenz), Kanalnamen und Gruppen-Pläne. "
       "Das WLAN-Passwort ist nicht enthalten; die SSID wird beim Import nicht angewendet.</p>";
  h += "<a class='btnlink' href='/export'>Export herunterladen</a>";
  h += "<form action='/import' method='POST' class='mt'>"
       "<label class='muted'>Import (exportierten Text einfügen):</label>"
...
void handleRoot() {
  if (currentMode == OP_AP) {
    ...
    server.send(200, "text/html; charset=utf-8", buildApConfigHtml("")); return;
  }
  if (!checkAuth()) return;
...
void handleSetWifi() {
  ...
  if (!server.hasArg("ssid") || !server.hasArg("pass")) {
    server.send(400,"text/html; charset=utf-8", buildApConfigHtml("SSID und Passwort erforderlich")); return; }
  String ssid = server.arg("ssid"), pass = server.arg("pass");
  if (ssid.length() < SSID_MIN_LEN || ssid.length() > SSID_MAX_LEN) {
    server.send(400,"text/html; charset=utf-8", buildApConfigHtml("SSID-Länge ungültig (1-32)")); return; }
  if (pass.length() < PASS_MIN_LEN || pass.length() > PASS_MAX_LEN) {
    server.send(400,"text/html; charset=utf-8", buildApConfigHtml("Passwort-Länge ungültig (8-63)")); return; }
...
void handleTokenAdd() {
  ...
  String t = genToken();
  strncpy(apiTokens[slot], t.c_str(), API_TOKEN_LEN); apiTokens[slot][API_TOKEN_LEN]='\0';
  persistApiTokens();
...
void handleTokenDelete() {
  ...
  apiTokens[idx][0] = '\0'; persistApiTokens();
...
void handleStart() {
  ...
  enqueueTask((uint8_t)server.arg("ch").toInt(), (uint16_t)server.arg("sec").toInt());
...
void handleWinter() {
  ...
  server.sendHeader("Location","/winter"); server.send(303);
}
void handleSetGroup() {
  if (!checkAuth()) return;
  ...
  G.stepCount = cnt;
  G.lastRunDay = 0; G.lastRunMin = 0xFFFF;
  saveGroups();
...
void handleClearGroup() {
  ...
  groups[slot].lastRunMin = 0xFFFF; groups[slot].name[GROUP_NAME_MAX] = '\0';
  saveGroups();
...
void handleApiStatus() {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    String nm = channelName(i); nm.replace("\\","\\\\"); nm.replace("\"","\\\"");
...
void setup() {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    digitalWrite(CHANNEL_PINS[i], RELAY_ACTIVE_LOW ? HIGH : LOW);
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) Serial.println("[OLED] Init fehlgeschlagen");
  else {
    display.clearDisplay(); display.setTextSize(1); display.setTextColor(SSD1306_WHITE);
    display.setCursor(0,0); display.println("Bewaesserung"); display.println("startet..."); display.display();
  }
...
void loop() {
  ...
  if (!ntpSynced && WiFi.status() == WL_CONNECTED) {
    struct tm ti;
    if (getLocalTime(&ti, 50)) { ntpSynced = true; Serial.println("[NTP] späte Sync ok"); }
  }
